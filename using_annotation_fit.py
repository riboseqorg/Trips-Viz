...
from core_functions import fetch_studies, fetch_files, fetch_file_paths
import pandas as pd
...
def get_genes_for_organism(connection, coding=False):
    ...
    if coding:
        result = curs.execute(
            "SELECT DISTINCT gene FROM transcripts WHERE gene_type == 1"
        ).fetchall()
    else:
        result = curs.execute(
            "SELECT DISTINCT gene FROM transcripts").fetchall()
...
def get_base_coverage(reads, cutoff_list, tranlen):
    ...
    for coverage_limit in cutoff_list:
        covered_positions = [
            position for position in reads[0]
            if reads[0][position] >= coverage_limit
        ]
        proportion_covered = round(
            float(len(covered_positions)) / float(tranlen), 2)
...
pcr = False
sqlite = '/home/DATA/www/tripsviz/tripsviz/trips_annotations/homo_sapiens/homo_sapiens.Gencode_v25.sqlite'


def region_scores_calculation_to_csv(sqlite, output, ambig, minread, maxread,
                                     ribocoverage, organism, transcriptome,
                                     noisered, primetype, readscore,
                                     secondary_readscore, pcr):
    data = {
        'five_prime_counts': [],
        'merged_length': [],
        'coding_counts': [],
    ...
    gene_list = get_genes_for_organism(connection, coding=True)
    ...
    for gene_number, gene in enumerate(gene_list):
        ...
        for transcript in gene_info:
            ...
            if gene_number % 10 == 0: df.to_csv(output)
            ...
            reads = get_reads(ambig, minread, maxread, transcript[0],
                              file_paths_dict, tranlen, ribocoverage, organism,
                              True, noisered, primetype, "riboseq", readscore,
                              secondary_readscore, pcr)
            annotation_fit_res = annotation_fit(reads, transcript[0],
                                                connection)
            annotation_fit_res["gene"] = gene
            annotation_fit_res["transcript"] = transcript[0]
            try:
                annotation_fit_res["annotation_fit"] = round(
                    float(annotation_fit_res['coding_counts']) /
                    float(annotation_fit_res['total_reads']), 2) * 100
                annotation_fit_res["merged_fit"] = round(
                    float(annotation_fit_res['merged_counts']) /
                    float(annotation_fit_res['total_reads']), 2) * 100
            ...
            try:
                annotation_fit_res['in_frame_score'] = float(
                    annotation_fit_res["in_frame_counts"]) / float(
                        annotation_fit_res["coding_counts"])
            except ZeroDivisionError:
            ...
            for proportion_limit_pair in coverage_scores:
                var_string = "covered_bases_limit_" + str(
                    proportion_limit_pair[1])
    ...
    for gene in genes:
        ...
        for index, row in gene_df.iterrows():
            if row["transcript"] not in gene_scores:
                gene_scores[row['transcript']] = {
                    "annotation_fit": 0,
                    "merged_fit": 0
                }
                try:
                    gene_scores[row['transcript']]["annotation_fit"] = round(
                        row['coding_counts'] / row['total_reads'], 2) * 100
                except:
                    continue
                try:
                    gene_scores[row['transcript']]["merged_fit"] = round(
                        row['merged_counts'] / row['total_reads'], 2) * 100
                except:
                    continue
            if row['transcript'] not in ouptut_dict:
                ouptut_dict[row['transcript']] = dict(row)
                ouptut_dict[row['transcript']]['annotation_fit'] = gene_scores[
                    row['transcript']]['annotation_fit']
...
region_scores_calculation_to_csv(sqlite, "test_out.csv", ambig, minread,
                                 maxread, ribocoverage, organism,
                                 transcriptome, noisered, primetype, readscore,
                                 secondary_readscore, pcr)
