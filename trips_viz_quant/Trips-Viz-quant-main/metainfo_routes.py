...
def create_custom_metagene(custom_seq_list, exclude_first_val,
                           exclude_last_val, include_first_val,
                           include_last_val, custom_search_region,
                           exclude_first, exclude_last, include_first,
                           include_last, sqlite_db, organism,
    ...
    if os.path.isfile("{0}{1}/{1}.{2}.sqlite".format(config.ANNOTATION_DIR,
...
def metainfoquery():
    ...
    if plottype == "sw_diff":
        ...
        traninfo_connection = sqlite3.connect(
            "/home/DATA/www/tripsviz/tripsviz/trips_annotations/{0}/{0}.{2}.sqlite"
        ...
        for file_id in sw_diff_group1:
            ...
            sqlite_db = SqliteDict(
                grp1_file_paths_dict["riboseq"][int(file_id)])
        ...
        for file_id in sw_diff_group2:
            ...
            sqlite_db = SqliteDict(
                grp2_file_paths_dict["riboseq"][int(file_id)])
            offsets = sqlite_db["offsets"]["fiveprime"]["offsets"]
            #print"offsets", offsets
            for transcript in trandict.keys():
    ...
    if plottype == "mismatch_pos":
            ...
            for file_id in file_paths_dict[filetype]:
                ...
                if os.path.isfile(filepath):
                    ...
                    if mapped_reads < 100000:
                        return "ERROR: File {} has less than 100,000 mapped reads".format(
                            filename)
                ...
                master_list.append(
                    (file_id, filename, range1_tpm, mapped_reads, file_desc))
        study_master_list = []
        ...
        if owner == 1:
            traninfo_dict = SqliteDict("{0}{1}/{1}.sqlite".format(
        ...
        if organism == "homo_sapiens" or organism == "homo_sapiens_polio":
            longest_tran_db = SqliteDict(
                "{0}homo_sapiens/principal_isoforms_5ldr3tlr_rnaseq.sqlite".
                format(config.ANNOTATION_DIR),
                autocommit=False)
        ...
        if owner == 1:
            ...
            transhelve = sqlite3.connect(
                "{0}transcriptomes/{1}/{2}/{3}/{2}_{3}.v2.sqlite".format(
                    config.UPLOADS_DIR, owner, organism, transcriptome))
            print "{0}transcriptomes/{1}/{2}/{3}/{2}_{3}.v2.sqlite".format(
                config.UPLOADS_DIR, owner, organism, transcriptome)
        cursor = transhelve.cursor()
        ...
        for filetype in file_paths_dict:
            for file_id in file_paths_dict[filetype]:
                ...
                if "frequent_unmapped_reads" not in sqlite_db:
                    return "No unmapped reads data for {}, please report this to tripsvizsite@gmail.com or via the contact page.".format(
                        filepath.split("/")[-1])
        ...
        studyname = ""
        top_reads = (sorted(master_dict.items(),
        ...
        for tup in top_reads[::-1]:
            html_table += (
                "<tr><td>{0}</td><td>{1}</td>    <td><a href='https://blast.ncbi.nlm.nih.gov/Blast.cgi?PROGRAM=blastn&PAGE_TYPE=BlastSearch&LINK_LOC=blasthome&QUERY=%3E{2}_unmapped_sequence%0A{0}' target='_blank'>Blast</a></td></tr>"
                .format(tup[0], tup[1], studyname))
        html_table += ("</table>")
        connection.close()
        return html_table
                ...
                for i in range(0, nuc_maxreadlen):
                    if i in master_count_dict[nuc]:
                        ...
                        if nuc_comp_type == "nuc_comp_per":
                            master_dict[nuc][i] = float(thiscount) / float(
                                othercount)
            ...
            for file_id in file_paths_dict[filetype]:
                ...
                if metagene_type == "metagene_start":
                    ...
                    mgc = create_custom_metagene(
                        ...
                        metagene_tranlist, metagene_frame)
...
def aggregate_counts(file_paths_dict, traninfo_dict, longest_tran_list, region,
    ...
    for transcript in final_tranlist:
        ...
        input_list = [
            "Aggregate", gene, transcript, region, riboseq_count, rnaseq_count,
            te
        ]
        for seq_type in all_seq_types:
            if seq_type != "riboseq" and seq_type != "rnaseq":
