...
single_transcript_plotpage_blueprint = Blueprint(
    "interactiveplotpage", __name__, template_folder="templates")
...
def interactiveplotpage(organism, transcriptome):
    ...
    cursor.execute("SELECT gwips_clade,gwips_organism,gwips_database,default_transcript from organisms WHERE organism_name = '{}' and transcriptome_list = '{}';".format(
...
single_transcript_query_blueprint = Blueprint(
    "query", __name__, template_folder="templates")
...
def query():
    ...
    if owner == 1:
        if os.path.isfile("{0}{1}/{1}.{2}.sqlite".format(config.ANNOTATION_DIR, organism, transcriptome)):
            ...
            transhelve = sqlite3.connect(
                "{0}{1}/{1}.{2}.sqlite".format(config.ANNOTATION_DIR, organism, transcriptome))
        else:
        ...
        transhelve = sqlite3.connect("{0}transcriptomes/{1}/{2}/{3}/{2}_{3}.v2.sqlite".format(
            config.UPLOADS_DIR, owner, organism, transcriptome))
            ...
            if len(result) == 1:
                ...
                if len(file_paths_dict["riboseq"].values()) > 0:
                    pre_orfQuant_res = incl_OPM_run_orfQuant(
                        tran, sqlite_path_organism, file_paths_dict["riboseq"].values())
                    pre_TPM_Ribo = TPM(
                        tran, sqlite_path_organism, file_paths_dict["riboseq"].values(), "ribo")

                    max_TPM_Ribo = max(pre_TPM_Ribo.values())
                    TPM_Ribo = {transcript: round(
                        (pre_TPM_Ribo[transcript] / max_TPM_Ribo)*100, 2) for transcript in pre_TPM_Ribo}

                    max_orf = max(pre_orfQuant_res.values())
                    orfQuant_res = {transcript: round(
                        (pre_orfQuant_res[transcript] / max_orf)*100, 2) for transcript in pre_orfQuant_res}

                else:
                    orfQuant_res = {
                        transcript[0]: "Null" for transcript in result}
                ...
                if len(file_paths_dict["rnaseq"].values()) > 0:
                    pre_TPM_RNA = TPM(tran, sqlite_path_organism,
                                      file_paths_dict["rnaseq"].values(), "rna")
                    max_TPM_RNA = max(pre_TPM_RNA.values())
                    TPM_RNA = {transcript: round(
                        (pre_TPM_RNA[transcript] / max_TPM_RNA)*100, 2) for transcript in pre_TPM_RNA}
                ...
                for transcript in result:
                    cursor.execute(
                        "SELECT length,cds_start,cds_stop,principal,version from transcripts WHERE transcript = '{}'".format(transcript[0]))
                    ...
                    return_str += (":{},{},{},{},{},{},{},{},{}".format(
                        transcript[0], version, tranlen, cds_start, cdslen, threeutrlen, OPM_coverage, ribo_coverage, RNA_coverage))
    ...
    if data["user_short"] == "None" or user_short_passed == True:
        short_code = generate_short_code(
            data, organism, data["transcriptome"], "interactive_plot")
    else:
    ...
    seq_rules = {"proteomics": {"frame_breakdown": 1}, "conservation": {
        "frame_breakdown": 1}, "tcpseq": {"frame_breakdown": 0}}
