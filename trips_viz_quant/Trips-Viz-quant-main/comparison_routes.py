            ...
            if len(result) == 1:
                ...
                if len(file_paths_dict["riboseq"]) > 0:
                    ...
                    pre_orfQuant_res = incl_OPM_run_orfQuant(
                        tran, sqlite_path_organism, file_paths_dict["riboseq"])
                    pre_TPM_Ribo = TPM(tran, sqlite_path_organism,
                                       file_paths_dict["riboseq"], "ribo")

                    max_TPM_Ribo = max(pre_TPM_Ribo.values())
                    TPM_Ribo = {
                        transcript:
                        round((pre_TPM_Ribo[transcript] / max_TPM_Ribo) * 100,
                              2)
                    ...
                    orfQuant_res = {
                        transcript:
                        round((pre_orfQuant_res[transcript] / max_orf) * 100,
                              2)
