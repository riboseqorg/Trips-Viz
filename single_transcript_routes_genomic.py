...
single_transcript_plotpage_genomic_blueprint = Blueprint("interactiveplotpage_genomic", __name__, template_folder="templates")
@single_transcript_plotpage_genomic_blueprint.route('/<organism>/<transcriptome>/interactive_plot_genomic/')
def interactiveplotpage_genomic(organism,transcriptome):
	...
	if consent == "deny":
		...
		return resp
	return render_template('index_genomic.html', gwips_info=gwips_info, gwips_clade=gwips_clade, gwips_org=gwips_org, gwips_db=gwips_db,organism=organism,transcriptome=transcriptome,default_tran=default_tran,
						   user_transcript=user_transcript, user_readscore=user_readscore, user_hili_starts=user_hili_starts, user_hili_stops=user_hili_stops,local=local,studies_dict=accepted_studies,
						   accepted_files=accepted_files,user_files=user_files,user_ribo_studies=user_ribo_studies,user_proteomics_studies=user_proteomics_studies, user_rna_studies=user_rna_studies,user_minread=user_minread,user_maxread=user_maxread,
						   user_dir=user_dir,user_line_graph=user_line_graph,user_ambig=user_ambig,user_cov=user_cov,user_nuc=user_nuc,user_short=user_short, user_crd=user_crd,studyinfo_dict=studyinfo_dict,
						   advanced=advanced,seq_types=seq_types)
...
single_transcript_query_genomic_blueprint = Blueprint("query_genomic", __name__, template_folder="templates")
@single_transcript_query_genomic_blueprint.route('/query_genomic', methods=['POST'])
def query_genomic():
	#global user_short_passed
	try:
