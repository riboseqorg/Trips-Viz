...
$(document).ready(() => {
  // Toggles project and samples visibility

  autoproject("file_type");
  $('input[name="file_type"]').on("click", () => {
    autoproject("file_type");
  });
  $('button[name="checkall"]').on("click", () => {
    const file_type = $('input[name = "file_type"]:checked').val();
    $(`.${file_type}`)
      .find(":checkbox")
      .each((_, element) => {
...
function formToJSON(form) {
  const array = $(form).serializeArray(); // Encodes the set of form elements as
