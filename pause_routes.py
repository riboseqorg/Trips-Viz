...
pause_detection_blueprint = Blueprint("pause_detection_page", __name__, template_folder="templates")
...
def pause_detection_page(organism,transcriptome):
	...
	return render_template('pause_detection.html', studies_dict=accepted_studies, accepted_files=accepted_files,user=user,default_tran="",local=local,advanced=advanced,
						   seq_types=seq_types,studyinfo_dict=studyinfo_dict,html_args=html_args,organism=organism,transcriptome=transcriptome)
...
def create_profiles(file_paths_dict,accepted_transcript_list,total_files,min_read_length,max_read_length):
	ambig = False
	minscore = 0
	file_count = 0
	...
	score_dict = {}
	seq_types = ["riboseq","proteomics"]
	color_list = ["%23ff0000","%232bff00","%230004ff","%23ffa200","%23c800ff","%23000000","%23969696","%23fa00f2"]
	color_ind = 0
	profile_dict = {}
	...
	for seq_type in seq_types:
		...
		for file_id in file_paths_dict[seq_type]:
			...
			file_count += 1
			file_name = (file_paths_dict[seq_type][file_id].split("/")[-1]).replace(".sqlite","").replace("_"," ")
			...
			for transcript in accepted_transcript_list:
				if transcript not in profile_dict[file_id]:
					profile_dict[file_id][transcript] = {"riboseq":{},"proteomics":{}}
				try:
					counts = sqlite_db[transcript]
				except Exception as e:
					continue
				if seq_type == "riboseq":
					subprofile = build_profile(counts, offsets,ambig,minscore=None,scores=scores)
				elif seq_type == "proteomics":
					subprofile = build_proteomics_profile(counts,ambig)
				for pos in subprofile:
					try:
						profile_dict[file_id][transcript][seq_type][pos] += subprofile[pos]
					except:
						profile_dict[file_id][transcript][seq_type][pos] = subprofile[pos]
		sqlite_db.close()
...
def extract_values(traninfo_dict,data,tran_gene_dict,selected_seq_types,profile_dict,min_fold_change,window,min_coverage,nuc_output):
	step = int(window/2)
	all_values_dict = {}
	file_output_dict = {}
		...
		for tran in profile_dict[file_id]:
			...
			for pos in score_dict:
				...
				if all_over_min == True:
					...
					all_values_dict[tran][pos][file_id] = [gene,tran, pos,avg_score, seq[pos-nuc_output:pos],seq[pos:pos+nuc_output], avg_cov, count,region]
	all_values_list = []
		...
		for pos in all_values_dict[tran]:
			if len(all_values_dict[tran][pos]) == file_number:
				...
				for file_id in all_values_dict[tran][pos]:
					...
					file_output_dict[file_id].append([gene, tran, pos,all_values_dict[tran][pos][file_id][3], useq, dseq,all_values_dict[tran][pos][file_id][6],all_values_dict[tran][pos][file_id][7],region])
				avg_score = tot_score/file_number
				avg_cov = tot_cov/file_number
				avg_count = tot_count/file_number
				all_values_list.append([gene, tran, pos, avg_score, useq, dseq, avg_cov, avg_count, region])
				
	sorted_all_values = sorted(all_values_list, key=lambda x: x[3],reverse=True)
	return (sorted_all_values, file_output_dict)




def write_to_file(sorted_all_values, file_output_dict,sequence_dict,organism,transcriptome,file_string,label_string,short_code):
	...
	tmp_result_file.write("Gene,Tran,Position,Region, Coverage,Pause Score,Upstream_sequence, Downstream_sequence,Count,Link\n")
	tup_count = 0
	...
	for tup in sorted_all_values:
		...
		comparison_url =  "https://trips.ucc.ie/{}/{}/comparison/?files={}{}&transcript={}&normalize=F&cov=T&ambig=F&minread=25&maxread=150&hili_start={}&hili_stop={}".format(organism,transcriptome,file_string,label_string,transcript,position-15,position+15)
		...
		#print (old_ebc_link)
		#print (ebc_link)
		tmp_result_file.write("{},{},{},{},{},{},{},{},{},{}\n".format(gene,transcript,position,region,cov,pause_score,upstream_seq,downstream_seq,count,ebc_link))
		if tup_count < 1000:
			returnstr += "{},{},{},{},{},{},{},{},{}.,/".format(gene,transcript,position,region,pause_score,upstream_seq,downstream_seq,count,ebc_link)
		tup_count += 1	
	tmp_result_file.close()
	#Create a zip file of all output files
	print ("zip -j {}/static/tmp/{}.zip {}".format(config.SCRIPT_LOC, short_code, all_filepaths))
	subprocess.call("zip -j {}/static/tmp/{}.zip {}".format(config.SCRIPT_LOC, short_code, all_filepaths),shell=True)
	return returnstr
...
def find_pauses(data,user,logged_in):
	...
	if html_args["user_short"] == "None":
		short_code = generate_short_code(data,organism,data["transcriptome"],"pause_pred")
	else:
	...
	profile_dict,file_string,label_string = create_profiles(file_paths_dict,principal_transcripts,total_files,min_read_length,max_read_length)
	sorted_all_values, file_output_dict = extract_values(traninfo_dict,data,tran_gene_dict,selected_seq_types,profile_dict,min_fold_change,window,min_coverage,nuc_output)
	...
	returnstr = write_to_file(sorted_all_values,file_output_dict,sequence_dict,organism,transcriptome,file_string,label_string,short_code)
...
pausequery_blueprint = Blueprint("pausequery", __name__, template_folder="templates")
