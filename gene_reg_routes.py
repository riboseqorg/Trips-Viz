...
def gene_regulationpage(organism, transcriptome):
    ...
    if user != None:
        cursor.execute(
            "SELECT advanced from users WHERE username = '{}';".format(user))
    ...
    for filename in os.listdir(
            "/home/DATA/www/tripsviz/tripsviz/gene_reg_tables"):
        if filename.endswith(".csv"):
            gene_reg_tables.append(filename.strip(".csv"))

    return render_template('gene_regulation.html',
                           studies_dict={},
                           accepted_files={},
                           user=user,
                           organism=organism,
                           ...
                           seq_types={},
                           studyinfo_dict={},
                           gene_reg_tables=gene_reg_tables)


def parse_csv_file(filepath, filename):
    openfile = open(filepath).readlines()
    table_html = "<table style  class='prediction_table hover'><tr><thead><th>File name {}</th></thead></tr>".format(
        filename)
    dict_user = {}
    count = 5
    for row in openfile:
    ...
    dict1 = {}
    files_human = os.listdir(
        "/home/DATA/www/tripsviz/tripsviz/z_combine_ribo/human")
    files_mouse = os.listdir(
        "/home/DATA/www/tripsviz/tripsviz/z_combine_ribo/mouse")

    keys_user2 = {}
    #print keys_user
    if len(set(dict_ortho.keys()).intersection(keys_user)) > len(
            set(dict_ortho.values()).intersection(keys_user)):
        mouse = 0
        table_html += "<tr><td>Genes identifed as Human</td></tr>"
        for filea in files_human:
            infileopen = open(
                "/home/DATA/www/tripsviz/tripsviz/z_combine_ribo/human/%s" %
                filea)
            dict1[filea] = {}
            infileopen.readline()
            for line in infileopen:
        ...
        for filea in files_mouse:
            infileopen = open(
                "/home/DATA/www/tripsviz/tripsviz/z_combine_ribo/mouse/%s" %
                filea)
            dict1[filea] = {}
            infileopen.readline()
            for line in infileopen:
                linesplit = line.split("\t")
                if linesplit[1] != "":
                    dict1[filea][linesplit[1]] = (
                        float(linesplit[2])
                    )  #for mouse data (i only use orthologues)
    else:
        mouse = 1
        ...
        for filea in files_human:
            ...
            for line in infileopen:
                ...
                if linesplit[1] != "":
                    dict1[filea][dict_ortho[linesplit[1]]] = (float(
                        linesplit[2]))
        for filea in files_mouse:
            infileopen = open(
                "/home/DATA/www/tripsviz/tripsviz/z_combine_ribo/mouse/%s" %
    ...
    if genes_used < 10000:
        table_html += "<tr><thead><th><font color='red'>{} <font color='red'>{}</th><th>Comparison may be unreliable</th></thead></tr>".format(
            "Genes recognised", genes_used)
        table_html += "<tr><thead><th><font color='red'>{}  <font color='red'>{}</th></thead></tr>".format(
            "Gene discarded", genes_discarded)
    else:
        table_html += "<tr><thead><th>Gene recognised {}</th></thead></tr>".format(
            genes_used)

        table_html += "<tr><thead><th>Gene discarded {}</th></thead></tr>".format(
            genes_discarded)
            ...
            if len(common_keys) > 2:
                ...
                if pvale2a < 0.05:
                    ...
                    if 1:
                        ...
                        #description_d =
                        out_data2.append(
                            (round(pvale1a, 3), round(np.median(listaa),
                                                      2), line_list, pvale2a,
                             line.split("\t")[0]))
    ...
    for a, b, c, d, e in out_data2:
        ...
        if a > 0.50 and abs(b) > 1:
            table_html += "<tr><td><font color='blue'>{}</td><td><font color='blue'>{:.4e}</td><td><font color='blue'>{}</td><td><font color='blue'>{}</td><td><font color='blue'>{}</td></tr>".format(
                e, d, a, b, c)
        #elif a  >.50 :
        ...
        #table_html += "<tr><td>{}</td><td>{}</td></tr>".format(splitrow[0],splitrow[1])
            ...
            table_html += "<tr><td>{}</td><td>{:.4e}</td><td>{}</td><td>{}</td><td>{}</td></tr>".format(
                e, d, a, b, c)
            ...
            if len(common_keys) > 2:
                ...
                if pvale2a < 0.05:
                    ...
                    if pvale1a >= 0.1:
                        ...
                        out_data2.append(
                            (round(pvale1a, 3), round(np.median(listaa), 2),
                             gene_list, pvale2a, line_list[0]))
    out_data2.sort()
    out_data2.reverse()
    for d, b, c, a, e in out_data2:
        if d > 0.5 and abs(b) > 1:
            table_html += "<tr><td>	<font color='blue'>{}</td><td><font color='blue'>{:.2e}</td><td>	<font color='blue'>{}</td><td><font color='blue'>{}</td><td>	<font color='blue'>{}</td></tr>".format(
                e, a, d, b, c)
        else:
            table_html += "<tr><td>	{}</td><td>{:.2e}</td><td>{}</td><td>{}</td><td>{}</td></tr>".format(
                e, a, d, b, c)
    ...
    for a, c, b, d in out_data:
        ...
        if c < 1:
            ...
            if a >= 0.3:
                table_html += "<tr><td><font color='blue'>{}</td><td>{}</td><td><font color='blue'>{}</td><td>+</td><td><font color='blue'>{}</td</tr>".format(
                    b, round(a, 4), round(d, 4), dict_des[b])
            else:
                table_html += "<tr><td>{}</td><td>{}</td><td>{}</td><td>+</td><td>{}</td</tr>".format(
                    b, round(a, 4), round(d, 4), dict_des[b])
        else:
            if a >= 0.3:
                table_html += "<tr><td><font color='blue'>{}</td><td>{}</td><td><font color='blue'>{}</td><td>-</td><td><font color='blue'>{}</td</tr>".format(
...
gene_regulation_query = Blueprint("gene_regulation_query",
                                  __name__,
...
def generegulationquery():
    ...
    if request.method == 'POST':
        ...
        for f in uploaded_files:
            ...
            upload_file_path = "{}/static/tmp/{}".format(
        ...
        try:
            ...
            tablehtml = parse_csv_file(
                "/home/DATA/www/tripsviz/tripsviz/gene_reg_tables/{}.csv".
                format(selected_file), selected_file)
            return tablehtml
        except:
            pass
    connection = sqlite3.connect(
        ...
        if 0:
            for file1 in ["Ribo", "TE", "RNA"]:
                ...
                for line in infileopen:
                    ...
                    if set(gene_list) - set(line_list) == set([]):
                        secondary_tablestr += "{}:{}.,/".format(
                            file1, ", ".join(line_list))
    ...
    #returnstr += "{},{},None,{},{}.,/".format(study, z_score,stra,annotation_popup)
        ...
        for filea in list_file2:
            ...
            if len(quer2) > 0:
                ...
                ideal_score = min([
                    mannwhitneyu(quer1, quer2a)[1],
                    mannwhitneyu(quer1, quer2b)[1]
                ])
                #if dop < rna_score:
                if 1:
                    ...
                    list_file.append(
                        (max(dop), dop, filea,
                         round(percentileofscore(quer1, abs(max(quer2))),
                               2), ideal_score, len(quer2), change2b,
                         change2ribo, change2rna))
            #quer1.sort()
        ...
        for study in list_file:
            ...
            if 1:
                ...
                changestr = "{}_{}_{}".format(study[6][0], study[6][1],
                                              missing)
                missing = study[7][2] - study[7][1] - study[7][0]
                changestr_ribo = "{}_{}_{}".format(study[7][0], study[7][1],
                                                   missing)
                missing = study[8][2] - study[8][1] - study[8][0]
                changestr_rna = "{}_{}_{}".format(study[8][0], study[8][1],
                                                  missing)

                #returnstr += "{},{},{},{},{},{},{}.,/".format(dict_name[study[1]], dict_pmid[study[1].split("_")[0]],study[0],study[2],changestr,stra,annotation_popup)
                returnstr += "{},{},{},{},{},{},{},{},{}.,/".format(
                    dict_name[study[2]], dict_pmid[study[2].split("_")[0]],
                    study[1][0], changestr, study[1][1], changestr_ribo,
                    study[1][2], changestr_rna, annotation_popup)
