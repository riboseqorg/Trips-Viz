...
const codon2amino = (str, frame) => slideStr(
                                        3,
                                        3,
                                        )(str.slice(frame))
...
const amino_colors = new Map([
  [ "A", "amber" ],   [ "R", "red" ],    [ "N", "blue" ],   [ "D", "purple" ],
  [ "C", "green" ],   [ "Q", "orange" ], [ "E", "yellow" ], [ "G", "brown" ],
  [ "H", "pink" ],    [ "I", "grey" ],   [ "L", "cyan" ],   [ "K", "black" ],
  [ "M", "magenta" ], [ "F", "white" ],  [ "P", "indigo" ], [ "S", "lime" ],
  [ "T", "maroon" ],  [ "W", "olive" ],  [ "Y", "navy" ],   [ "V", "teal" ],
  [ "*", "red" ],
]);
...
function rdg_plot(data) {
  ...
  var svg = d3.select("#rdg").append("svg").attr("width", full_width - 50);
  var line = d3.select("#rdg").select("svg");
  console.log(d3.extent(data, (d) => +d.x2)[1]);
  const x =
      d3.scaleLinear()
          .domain([
            0, d3.extent(data, (d) => +d.x2)[1]
          ]) // + mean incremental // TODO:replace this with a fixed value later
          .range([ 0, full_width ]);
  var i = 0;
  var already_there = [];
  cds_count = 0;
  ...
  svg.attr("height", (cds_count + 1) * 10 + 10);
}
...
function line_plot(data) {
  ...
  const yScale = d3.scaleLinear().domain([ 0, ymax ]).range([
    full_height - margin.bottom - margin.top, 0
  ]);
  ...
  const xScale2 =
      d3.scaleBand()
          .range([ 0, full_width - (margin.left + margin.right) ])
          .domain(d3.range(0, d3.max(plot_data.map((d) => +d.pos)) + 1)
                      .map(String)) // + mean incremental
          // .rangeRound([0, 1000])
          .padding(0.02);
...
function cds_line(data, svg, xScale, yScale, ymax) {
  ...
  // cds start
  if (data.cds_range[0] >= xScale.domain()[0] &&
      data.cds_range[0] <= xScale.domain()[1]) {
    svg.append("line")
        .attr("class", "ccc")
  ...
  // cds stop
  if (data.cds_range[1] >= xScale.domain()[0] &&
      data.cds_range[1] <= xScale.domain()[1]) {
    svg.append("line")
        .attr("class", "ccc")
...
function cds_plot(data, svg, xScale) {
  ...
  data.forEach((dt) => {
    var d = structuredClone(dt);
    if ((d.coding_start < xScale.domain()[0] &&
         d.coding_stop < xScale.domain()[0]) ||
        (d.coding_start > xScale.domain()[1] &&
         d.coding_stop > xScale.domain()[1])) {
      d.coding_start = 0;
      d.coding_stop = 0;
    } else {
      if (d.coding_start < xScale.domain()[0] &&
          d.coding_stop > xScale.domain()[0]) {
        d.coding_start = xScale.domain()[0];
      }
      if (d.coding_start < xScale.domain()[1] &&
          d.coding_stop > xScale.domain()[1]) {
        d.coding_stop = xScale.domain()[1];
      }
    }
...
function aa_plot(data, svg, xScale) {
  ...
  for (let frame = 0; frame < 3; frame++) {
    ...
    chr_pixels = ((full_width - margin.left - margin.right) * 1.0) /
                 Math.ceil(ss[1] - ss[0]);
    ...
    if (chr_pixels < 7) {
      ...
      aa.split("").forEach((dt, i) => {
        var posa = frame + i * 3 + 1;
        if (posa >= xScale.domain()[0] && posa <= xScale.domain()[1]) {
          g.append("text")
              ...
              .text(dt)
              .attr("fill",
                    amino_colors.get(dt)); // TODO: Add color based on frame.
        }
      });
    }
    g.append("text")
  ...
  if (chr_pixels < 14) {
    str.split("").forEach((dt, i) => {
      if (i + 1 >= xScale.domain()[0] && i + 1 <= xScale.domain()[1]) {
        g.append("line")
