...
def diffquery():
    ...
    for row in result:
        traninfo_dict[row[0]] = {
            ...
            "seq": row[5],
            "strand": row[6],
            "stop_list": row[7].split(","),
            "start_list": row[8].split(","),
            "exon_junctions": row[9].split(","),
            "tran_type": row[10],
            "principal": row[11]
        }
    ...
    riboseq2_filepaths = {}
    file_paths_dict = fetch_file_paths(
        master_file_dict["riboseq2"]["file_ids"], organism)
    for seq_type in file_paths_dict:
        for file_id in file_paths_dict[seq_type]:
    ...
    try:
        rnaseq1_filepaths = {}
        file_paths_dict = fetch_file_paths(
            master_file_dict["rnaseq1"]["file_ids"], organism)
        for seq_type in file_paths_dict:
            for file_id in file_paths_dict[seq_type]:
                rnaseq1_filepaths[file_id] = file_paths_dict[seq_type][file_id]
        rnaseq2_filepaths = {}
        file_paths_dict = fetch_file_paths(
            master_file_dict["rnaseq2"]["file_ids"], organism)
        for seq_type in file_paths_dict:
            for file_id in file_paths_dict[seq_type]:
    ...
    deseq_count_filename = "{}/static/tmp/{}_deseq_counts.csv".format(
        config.SCRIPT_LOC, filename)
    deseq_sample_filename = "{}/static/tmp/{}_deseq_sample_info.csv".format(
        config.SCRIPT_LOC, filename)
    ...
    if len(master_file_dict["riboseq1"]["file_ids"]) == 0 and len(
            master_file_dict["riboseq2"]["file_ids"]) == 0 and len(
                master_file_dict["rnaseq1"]["file_ids"]) == 0 and len(
                    master_file_dict["rnaseq2"]["file_ids"]) == 0:
    ...
    if len(master_file_dict["riboseq1"]["file_ids"]) != len(
            master_file_dict["riboseq2"]["file_ids"]):
        return prepare_return_str(
            "Error: Both Riboseq Condition boxes need to have an equal number of files."
        )
    if len(master_file_dict["rnaseq1"]["file_ids"]) != len(
            master_file_dict["rnaseq2"]["file_ids"]):
        return prepare_return_str(
            "Error: Both RNA-Seq Condition boxes need to have an equal number of files."
        )
    if len(master_file_dict["rnaseq1"]["file_ids"]) >= 1 and len(
            master_file_dict["riboseq1"]["file_ids"]) >= 1 and (len(
                master_file_dict["riboseq1"]["file_ids"]) != len(
                    master_file_dict["rnaseq1"]["file_ids"])):
        return prepare_return_str(
            "Error: If RNA-Seq boxes are not empty they need to have an equal number of files as the riboseq boxes"
        )
    if len(master_file_dict["riboseq1"]["file_ids"]) >= 1 and len(
            master_file_dict["riboseq2"]["file_ids"]) >= 1 and len(
                master_file_dict["rnaseq1"]["file_ids"]) >= 1 and len(
                    master_file_dict["rnaseq2"]["file_ids"]) >= 1:
        label = "TE"
    elif len(master_file_dict["riboseq1"]["file_ids"]) >= 1 and len(
            master_file_dict["riboseq2"]["file_ids"]) >= 1 and len(
                master_file_dict["rnaseq1"]["file_ids"]) == 0 and len(
                    master_file_dict["rnaseq2"]["file_ids"]) == 0:
        label = "Riboseq"
    elif len(master_file_dict["riboseq1"]["file_ids"]) == 0 and len(
            master_file_dict["riboseq2"]["file_ids"]) == 0 and len(
                master_file_dict["rnaseq1"]["file_ids"]) >= 1 and len(
                    master_file_dict["rnaseq2"]["file_ids"]) >= 1:
        label = "Rnaseq"
        ...
        label = "TE"
        return prepare_return_str(
            "ERROR IMBALANCED OR NO FILES: Either all 4 boxes (RIBO-seq files 1, RIBO-seq files 2, mRNA-seq files 1, mRNA-seq files 2) must have a file associated with it OR both riboseq boxes OR both rnaseq boxes, you currently have {} files in RIBO-seq condition 1 files, {} in RIBO-seq condition 2 files, {} in mRNA-seq condition 1 files and {} in mRNA-seq condition 2 files"
            .format(len(riboseq1_filepaths), len(riboseq2_filepaths),
                    len(rnaseq1_filepaths), len(rnaseq2_filepaths)))
    ...
    for i in range(0, no_groups):
        if label == "TE":
            csv_file.write(
                "Group,Fold change (log2),Geometric Mean (log2), Bin mean, Bin standard deviation, Z-score,"
            )
            csv_file.write(
                "RiboSeq_Cond1_count,RiboSeq_Cond2_count,mRNA-Seq_Cond1_count,mRNA-Seq_Cond2_count,"
            )
            if mapped_reads_norm:
                csv_file.write(
                    "Normalised_RiboSeq_Cond1_count,Normalised_RiboSeq_Cond2_count,Normalised_mRNA-Seq_Cond1_count,Normalised_mRNA-Seq_Cond2_count,"
                )
        elif label == "Riboseq":
            csv_file.write(
                "Group,Fold change (log2),Geometric Mean (log2), Bin mean, Bin standard deviation, Z-score,"
            ...
            if mapped_reads_norm:
                csv_file.write(
                    "Normalised_RiboSeq_Cond1_count ,Normalised_RiboSeq_Cond2_count,"
                )
        else:
            csv_file.write(
                "Group,Fold change (log2),Geometric Mean (log2), Bin mean, Bin standard deviation, Z-score,"
        ...
        if len(riboseq1_filepaths) != 0:
            ...
            riboseq2_filepath = riboseq2_filepaths[int(
                master_file_dict["riboseq2"]["file_ids"][i])]
        else:
            riboseq1_filepath = ""
            riboseq2_filepath = ""

        if len(rnaseq1_filepaths) != 0:
            rnaseq1_filepath = rnaseq1_filepaths[int(
                master_file_dict["rnaseq1"]["file_ids"][i])]
            rnaseq2_filepath = rnaseq2_filepaths[int(
                master_file_dict["rnaseq2"]["file_ids"][i])]
        else:
            rnaseq1_filepath = ""
            rnaseq2_filepath = ""

        transcript_dict, groupname = calculate_zscore(
            riboseq1_filepath, riboseq2_filepath, rnaseq1_filepath,
    ...
    if plottype == "deltate":
        ...
        deltate_sample_filename = "{}/static/tmp/{}/DELTATE_sample_info.csv".format(
            config.SCRIPT_LOC, deltate_folder)
        ...
        deltate_ribo_count_filename = "{}/static/tmp/{}/DELTATE_ribo_counts.csv".format(
            config.SCRIPT_LOC, deltate_folder)
        deltate_rna_count_filename = "{}/static/tmp/{}/DELTATE_rna_counts.csv".format(
            config.SCRIPT_LOC, deltate_folder)
        ...
        for i in range(1, no_groups + 1):
            if label in ["TE", "Riboseq"]:
                deltate_sample_file.write(
                    "Ribo_cond1_count{},1,RIBO\n".format(i))
        ...
        deltate_ribo_count_file.write("{}\n".format(
            str(ribo_headers).strip("[]").replace("'", "")))
        ...
        for tran in master_dict:
            ...
            deltate_ribo_count_file.write("{},{}\n".format(
                str(ribo1_counts).strip("[]"),
                str(ribo2_counts).strip("[]")))
            deltate_rna_count_file.write("{},{}\n".format(
                str(rna1_counts).strip("[]"),
                str(rna2_counts).strip("[]")))

        deltate_ribo_count_file.close()
        deltate_rna_count_file.close()
        print("{}/R/DTEG.R {} {} {} 0 {}".format(config.SCRIPT_LOC,
                                                 deltate_ribo_count_filename,
                                                 deltate_rna_count_filename,
                                                 deltate_sample_filename,
                                                 deltate_folder))
        subprocess.call("{}/R/DTEG.R {} {} {} 0 {}".format(
            config.SCRIPT_LOC, deltate_ribo_count_filename,
            deltate_rna_count_filename, deltate_sample_filename,
            deltate_folder),
                        shell=True)
    ...
    if plottype == "deseq2":
        ...
        for i in range(1, no_groups + 1):
            if label in ["TE", "Riboseq"]:
                deseq_sample_file.write(
                    "Ribo_cond1_count{},control,riboseq\n".format(i))
                headers.append("Ribo_cond1_count{}".format(i))
            sample_data.append(["control", "riboseq"])
            sample_rows.append("Ribo_cond1_count{}".format(i))
            if label in ["TE", "Riboseq"]:
                deseq_sample_file.write(
                    "Ribo_cond2_count{},treatment,riboseq\n".format(i))
                headers.append("Ribo_cond2_count{}".format(i))
            sample_data.append(["treatment", "riboseq"])
            sample_rows.append("Ribo_cond2_count{}".format(i))
            if label in ["TE", "Rnaseq"]:
                deseq_sample_file.write(
                    "mRNA_cond1_count{},control,rnaseq\n".format(i))
                headers.append("mRNA_cond1_count{}".format(i))
            sample_data.append(["control", "rnaseq"])
            sample_rows.append("mRNA_cond1_count{}".format(i))
            if label in ["TE", "Rnaseq"]:
                deseq_sample_file.write(
                    "mRNA_cond2_count{},treatment,rnaseq\n".format(i))
        ...
        for tran in master_dict:
            ...
            for group in master_transcript_dict:
                ribo1 = int(
                    (2**master_transcript_dict[group][tran]["riboseq1"]) -
                    0.0001)
                ribo2 = int(
                    (2**master_transcript_dict[group][tran]["riboseq2"]) -
                    0.0001)
                rna1 = int(
                    (2**master_transcript_dict[group][tran]["rnaseq1"]) -
                    0.0001)
                rna2 = int(
                    (2**master_transcript_dict[group][tran]["rnaseq2"]) -
                    0.0001)
                ...
                if label == "TE":
                    deseq_count_file.write("{},{},{},{}".format(
                        ribo1, ribo2, rna1, rna2))
                elif label == "Riboseq":
        ...
        subprocess.call("{}/R/deseq2_triplicates.R {} {} {} {} {}".format(
            config.SCRIPT_LOC, deseq_basename, deseq_count_filename,
            deseq_sample_filename, label, no_groups),
                        shell=True)
        if label == "TE":
            subprocess.call(
    ...
    if anota2seq:
        if label != "TE" or no_groups <= 2:
            return prepare_return_str(
                "At least 3 replicates are required for all groups when using Anota2seq"
            )
        for i in range(1, no_groups + 1):
            if label in ["TE", "Riboseq"]:
                deseq_sample_file.write(
        ...
        for tran in master_dict:
            ...
            deseq_count_file.write("\n")
        deseq_count_file.close()
        subprocess.call("{}/R/anota2seq.R {} {} {} {} {} {}".format(
            config.SCRIPT_LOC, deseq_basename, deseq_count_filename,
            deseq_sample_filename, label, no_groups, minzscore / 100),
                        shell=True)
        if label == "TE":
            subprocess.call(
                "tar -C {1}/static/tmp/ -czvf {1}/static/tmp/{0}.tar.gz  {0}_deseq_counts.csv {0}_deseq_sample_info.csv {0}_anota2seq_RIBOSEQ.csv {0}_anota2seq_BUFFERED.csv {0}_anota2seq_mRNA.csv"
                .format(filename, config.SCRIPT_LOC, deseq_count_filename,
                        deseq_sample_filename),
                shell=True)
        elif label == "Riboseq":
            subprocess.call(
                "tar -C {1}/static/tmp/ -czvf {1}/static/tmp/{0}.tar.gz  {0}_deseq_counts.csv {0}_deseq_sample_info.csv {0}_anota2seq_RIBOSEQ.csv"
                .format(filename, config.SCRIPT_LOC, deseq_count_filename,
                        deseq_sample_filename),
                shell=True)
        elif label == "Rnaseq":
            subprocess.call(
                "tar -C {1}/static/tmp/ -czvf {1}/static/tmp/{0}.tar.gz  {0}_deseq_counts.csv {0}_deseq_sample_info.csv {0}_anota2seq_mRNA.csv"
                .format(filename, config.SCRIPT_LOC, deseq_count_filename,
                        deseq_sample_filename),
                shell=True)
        # Parse TE file to get the x,y values of the transcripts and get the genes below a signifigcance level.
        ...
        sig_buffered = []
        if os.path.isfile(
                "{0}/static/tmp/{1}_anota2seq_sig_translated_genes.csv".format(
                    config.SCRIPT_LOC, filename)):
            sig_translated_file = open(
                "{0}/static/tmp/{1}_anota2seq_sig_translated_genes.csv".format(
                    config.SCRIPT_LOC, filename)).readlines()
        ...
        if os.path.isfile(
                "{0}/static/tmp/{1}_anota2seq_sig_rna_genes.csv".format(
                    config.SCRIPT_LOC, filename)):
            sig_rna_file = open(
                "{0}/static/tmp/{1}_anota2seq_sig_rna_genes.csv".format(
                    config.SCRIPT_LOC, filename)).readlines()
            for line in sig_rna_file[1:]:
                ...
                sig_rna.append(gene)
        if os.path.isfile(
                "{0}/static/tmp/{1}_anota2seq_sig_buffered_genes.csv".format(
                    config.SCRIPT_LOC, filename)):
            sig_buffered_file = open(
                "{0}/static/tmp/{1}_anota2seq_sig_buffered_genes.csv".format(
                    config.SCRIPT_LOC, filename)).readlines()
        ...
        if os.path.isfile("{0}/static/tmp/{1}_anota2seq_RIBOSEQ.csv".format(
                config.SCRIPT_LOC, filename)):
            ribo_file = open("{0}/static/tmp/{1}_anota2seq_RIBOSEQ.csv".format(
                config.SCRIPT_LOC, filename)).readlines()
            for line in ribo_file[1:]:
                splitline = line.split(",")
                ...
                if gene not in deseq_dict:
                    ...
                    deseq_dict[gene]["ribo_padj"] = padj
                    deseq_dict[gene]["ribo_fc"] = log2fc
        if os.path.isfile("{0}/static/tmp/{1}_anota2seq_mRNA.csv".format(
                config.SCRIPT_LOC, filename)):
            rna_file = open("{0}/static/tmp/{1}_anota2seq_mRNA.csv".format(
                config.SCRIPT_LOC, filename)).readlines()
            for line in rna_file[1:]:
                splitline = line.split(",")
                transcript = splitline[1].split("___")[1]
        ...
        return riboflask_diff.anota2seq_plot(
            deseq_dict, organism, transcriptome,
            master_file_dict["riboseq1"]["file_ids"],
            master_file_dict["riboseq2"]["file_ids"],
            master_file_dict["rnaseq1"]["file_ids"],
    ...
    for tran in master_dict:
        ...
        if (len(master_dict[tran].keys()) - 1) != no_groups:
            ...
            for group in master_transcript_dict:
                try:
                    if master_transcript_dict[group][tran][
    ...
    if no_groups == 2:
        ...
        for tran in master_dict:
            ...
            for groupname in master_dict[tran]:
                if groupname == "skip":
                    continue
                z_score = (master_dict[tran][groupname]["fold_change"] -
                           master_dict[tran][groupname]["mean"]
                           ) / master_dict[tran][groupname]["standard_dev"]
        ...
        for i in range(0, 10):
            ...
            for y in range(0, len(false_positives_z_dict[0])):
                ...
                average_z_score = sum(shuffled_z_scores) / len(
                    shuffled_z_scores)
        ...
        if all_shuffled_z_scores != []:
            ...
            for tran in master_dict:
                if "z_score" in master_dict[tran]:
                    ...
                    vala = float(
                        len([
                            1 for bb2 in all_genuine_z_scores
                            if bb2 >= abs(z_score)
                        ]))
                    try:
    ...
    for tran in master_dict:
        ...
        for groupname in master_dict[tran]:
            ...
            geo_mean_list.append(
                master_dict[tran][groupname]["geometric_mean"])
            fc_list.append(master_dict[tran][groupname]["fold_change"])
            fc = master_dict[tran][groupname]["fold_change"]
            z_score = (fc - master_dict[tran][groupname]["mean"]
                       ) / master_dict[tran][groupname]["standard_dev"]
            z_scores.append(z_score)
            geo_mean = master_dict[tran][groupname]["geometric_mean"]
            if label == "TE":
                csv_file.write("{},{},{},{},{},{},".format(
                    groupname, fc, geo_mean,
                    master_dict[tran][groupname]["mean"],
                    master_dict[tran][groupname]["standard_dev"], z_score))
                riboseq1_count = (2**transcript_dict[tran][
                    "riboseq1"]) * transcript_dict["ribo1_modifier"] - 0.0001
                riboseq2_count = (2**transcript_dict[tran][
                    "riboseq2"]) * transcript_dict["ribo2_modifier"] - 0.0001
                rnaseq1_count = (2**transcript_dict[tran]["rnaseq1"]
                                 ) * transcript_dict["rna1_modifier"] - 0.0001
                rnaseq2_count = (2**transcript_dict[tran]["rnaseq2"]
                                 ) * transcript_dict["rna2_modifier"] - 0.0001
                ...
                if rnaseq2_count < 0.1:
                    rnaseq2_count = 0
                csv_file.write("{},{},{},{},".format(riboseq1_count,
                                                     riboseq2_count,
                                                     rnaseq1_count,
                                                     rnaseq2_count))
                if gene not in ribo_vs_rna_dict:
                    ribo_vs_rna_dict[gene] = {
                        "tran": tran,
                    ...
                    ribo_vs_rna_dict[gene]["ribo1"] += 2**transcript_dict[
                        tran]["riboseq1"]
                    ribo_vs_rna_dict[gene]["ribo2"] += 2**transcript_dict[
                        tran]["riboseq2"]
                    ribo_vs_rna_dict[gene]["rna1"] += 2**transcript_dict[tran][
                        "rnaseq1"]
                    ribo_vs_rna_dict[gene]["rna2"] += 2**transcript_dict[tran][
                        "rnaseq2"]
                if mapped_reads_norm:
                    csv_file.write("{},{},{},{},".format(
                        (2**transcript_dict[tran]["riboseq1"]) - 0.0001,
                        (2**transcript_dict[tran]["riboseq2"]) - 0.0001,
                        (2**transcript_dict[tran]["rnaseq1"]) - 0.0001,
        ...
        average_z_score = sum(z_scores) / len(z_scores)
        master_dict[tran]["z_score"] = average_z_score
        absolute_zcores.append(
            (str(gene), abs(average_z_score), average_z_score))
    ...
    for i in range(0, len(sorted_aggregated_values), 300):
        ...
        if i < (len(sorted_aggregated_values) - 300):
            ...
            bin_list.append(
                [mean, standard_dev, upper_threshold, lower_threshold])
        else:
    ...
    connection.close()
    if plottype == "z_score":
        return riboflask_diff.generate_plot(
            sorted_aggregated_values, bin_list, organism, label, transcriptome,
            master_file_dict["riboseq1"]["file_ids"],
            master_file_dict["riboseq2"]["file_ids"],
            master_file_dict["rnaseq1"]["file_ids"],
            master_file_dict["rnaseq2"]["file_ids"], background_col,
            ...
            str(marker_size) + "pt", ambiguous, gene_list)

    else:
        return riboflask_diff.ribo_vs_rna(
            ribo_vs_rna_dict, organism, transcriptome,
            master_file_dict["riboseq1"]["file_ids"],
            master_file_dict["riboseq2"]["file_ids"],
            master_file_dict["rnaseq1"]["file_ids"],
            master_file_dict["rnaseq2"]["file_ids"], background_col,
...
def calculate_zscore(riboseq1_filepath: str, riboseq2_filepath: str,
    ...
    if riboseq1_filepath:
        groupname += (riboseq1_filepath.split("/")[-1]).replace(".sqlite",
                                                                "") + "_"
        if os.path.isfile(riboseq1_filepath):
            sqlite_db = SqliteDict(f"{riboseq1_filepath}",
                                   autocommit=False,
                                   decode=my_decoder)
        else:
        ...
        if min_cov:
            ...
            if region == "fiveprime":
                ...
                covdict = sqlite_db["{}_threeprime_coverage".format(
                    ambig_type)]
            elif region == "all":
        ...
        for transcript in longest_tran_list:
            ...
            if transcript in covdict:
                transcript_dict[transcript]["riboseq1_cov"] += covdict[
                    transcript]

    if riboseq2_filepath:
        groupname += (riboseq2_filepath.split("/")[-1]).replace(".sqlite",
                                                                "") + "_"
        if os.path.isfile(riboseq2_filepath):
            sqlite_db = sqlite_db = SqliteDict(f"{riboseq2_filepath}",
                                               autocommit=False,
                                               decode=my_decoder)
        else:
            return (
        ...
        for transcript in longest_tran_list:
            ...
            if transcript in covdict:
                transcript_dict[transcript]["riboseq2_cov"] += covdict[
                    transcript]

    if rnaseq1_filepath:
        groupname += (rnaseq1_filepath.split("/")[-1]).replace(".sqlite",
                                                               "") + "_"
        if os.path.isfile(rnaseq1_filepath):
            sqlite_db = SqliteDict(f"{rnaseq1_filepath}",
                                   autocommit=False,
                                   decode=my_decoder)
        else:
            return (
        ...
        for transcript in longest_tran_list:
            ...
            if transcript in covdict:
                transcript_dict[transcript]["rnaseq1_cov"] += covdict[
                    transcript]
    if rnaseq2_filepath:
        groupname += (rnaseq2_filepath.split("/")[-1]).replace(".sqlite",
                                                               "") + "_"
        if os.path.isfile(rnaseq2_filepath):
            sqlite_db = SqliteDict(f"{rnaseq2_filepath}",
                                   autocommit=False,
                                   decode=my_decoder)
        else:
            return (
        ...
        for transcript in longest_tran_list:
            ...
            if transcript in covdict:
                transcript_dict[transcript]["rnaseq2_cov"] += covdict[
                    transcript]
    ...
    for transcript in transcript_dict:
        if label == "TE":
            current_min_reads = min(
                transcript_dict[transcript]["riboseq1"] / ribo1_modifier,
                transcript_dict[transcript]["riboseq2"] / ribo2_modifier,
                transcript_dict[transcript]["rnaseq1"] / rna1_modifier,
                transcript_dict[transcript]["rnaseq2"] / rna2_modifier)
        elif label == "Riboseq":
            current_min_reads = min(
                transcript_dict[transcript]["riboseq1"] / ribo1_modifier,
                transcript_dict[transcript]["riboseq2"] / ribo2_modifier)
    ...
    for transcript in transcript_dict:
        if min_cov > 0:
            max_cov = max(transcript_dict[transcript]["riboseq1_cov"],
                          transcript_dict[transcript]["riboseq2_cov"],
                          transcript_dict[transcript]["rnaseq1_cov"],
                          transcript_dict[transcript]["rnaseq2_cov"])
        ...
        if transcript_dict[transcript]["riboseq1"] != 0:
            transcript_dict[transcript]["riboseq1"] = log(
                (transcript_dict[transcript]["riboseq1"] / ribo1_modifier), 2)
        if transcript_dict[transcript]["riboseq2"] != 0:
            transcript_dict[transcript]["riboseq2"] = log(
                (transcript_dict[transcript]["riboseq2"] / ribo2_modifier), 2)
        if transcript_dict[transcript]["rnaseq1"] != 0:
            transcript_dict[transcript]["rnaseq1"] = log(
                (transcript_dict[transcript]["rnaseq1"] / rna1_modifier), 2)
        if transcript_dict[transcript]["rnaseq2"] != 0:
            transcript_dict[transcript]["rnaseq2"] = log(
                (transcript_dict[transcript]["rnaseq2"] / rna2_modifier), 2)
