...
def metainfoquery():
	...
	if plottype == "sw_diff":
		...
		for tran in grp1_profile_dict:
			...
			if tran in grp2_profile_dict:
				tranlen = trandict[tran]
				for i in range(0,tranlen, sw_diff_step_size):
					#if i != 690:
					#	continue
					grp1_count = 1.0
					grp2_count = 1.0
	...
	if plottype == "mismatch_pos":
		...
		if owner == 1:
			...
			transhelve = sqlite3.connect("{0}{1}/{1}.v2.sqlite".format(config.ANNOTATION_DIR,organism))
			print "{0}{1}/{1}.v2.sqlite".format(config.ANNOTATION_DIR,organism)
		else:
		...
		longest_tran_list = traninfo_dict.keys()
		if count_agg == True:
			table_str = aggregate_counts(file_paths_dict, traninfo_dict, longest_tran_list, region, organism,all_seq_types, te_minimum_reads, html_args,count_type)
		elif count_agg == False:
			table_str = sample_counts(file_paths_dict, traninfo_dict, longest_tran_list, region, organism,all_seq_types, te_minimum_reads, html_args,count_type)
		return table_str
...
def aggregate_counts(file_paths_dict, traninfo_dict, longest_tran_list, region, organism, all_seq_types, te_minimum_reads, html_args,count_type):
	file_list = ""
	transcript_dict = {}
	table_str = ""
		...
		for transcript in longest_tran_list:
			...
			if traninfo_dict[transcript]["cds_start"] != None:
				if region == "fiveprime":
					...
					tranlength = float(traninfo_dict[transcript]["cds_stop"]-traninfo_dict[transcript]["cds_start"])
				elif region == "threeprime":
					tranlength = float(traninfo_dict[transcript]["length"]-traninfo_dict[transcript]["cds_stop"])
...
def sample_counts(file_paths_dict, traninfo_dict, longest_tran_list, region, organism, all_seq_types, te_minimum_reads, html_args,count_type):
	file_list = ""
	transcript_dict = {}
	table_str = ""
	filename = organism+"_translation_efficiencies_"+str(time.time())+".csv"
		...
		for transcript in longest_tran_list:
			...
			for seq_type in transcript_dict[transcript]:
				for inputfilename in transcript_dict[transcript][seq_type]:
					mapped_reads = mapped_reads_dict[inputfilename]
					#per million scaling factor
					pmsf = mapped_reads/1000000
