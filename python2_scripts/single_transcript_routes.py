...
def interactiveplotpage(organism,transcriptome):
	...
	print "user generate short url: ", user_generate_shorturl
			...
			if len(result) == 1:
				...
				for transcript in result:
					...
					if user == "test":
						...
						annotated_cds_reads = float(annotation_fit_results["coding_counts"])
						non_coding_reads = float(annotation_fit_results["five_prime_counts"] + annotation_fit_results["three_prime_counts"]) 
						coverage_limit = 10
						covered_positions = [position for position in reads[0] if reads[0][position] >= coverage_limit]
						covered_bases = float(len(covered_positions))/float(tranlen)
						...
						return_str += (":{},{},{},{},{},{},{},{},{},{},{}".format(transcript[0],version, tranlen, cds_start, cdslen, threeutrlen, round(annotated_cds_reads/total_reads, 2) * 100, round(merged_reads/total_reads, 2) * 100, round(covered_bases, 2), ribo_coverage, RNA_coverage))
					else:
