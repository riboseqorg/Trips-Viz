...
def metainfoquery():
	...
	if plottype == "bulk_dl":
		...
		traninfo_connection = sqlite3.connect(sqlite_path_organism)
		traninfo_cursor = traninfo_connection.cursor()
			...
			for file_id in file_paths_dict[filetype]:
				...
				for transcript in traninfo_dict:
						if transcript not in master_dict:
							master_dict[transcript] = {}
							for i in range(1,traninfo_dict[transcript]["length"]+1):
								master_dict[transcript][i] = 0
									...
									if readlen > minreadlen and readlen < maxreadlen:
										...
										for pos in counts[readlen]:
											if apply_offset == True:
												try:
													master_dict[transcript][pos+offset+1] += counts[readlen][pos]
												except:
													pass
											else:
												master_dict[transcript][pos+1] += counts[readlen][pos]
		
		outfile = SqliteDict("{}/static/tmp/bulk_dl_{}.sqlite".format(config.SCRIPT_LOC,curr_time))
		...
		return_str =  "<div style='padding-left: 55px;padding-top: 22px;'><a href='https://trips.ucc.ie/static/tmp/bulk_dl_{1}.sqlite' target='_blank' ><button class='button centerbutton' type='submit'><b>Download result</b></button></a> </div>".format(config.SCRIPT_LOC, curr_time)
		if app.debug == True or celery_availability == None:
			return return_str, "NO_CELERY", {'Location': None}
		else:
			return prepare_return_str(return_str)
...
def sample_counts(file_paths_dict, traninfo_dict, longest_tran_list, region, organism, all_seq_types, te_minimum_reads, html_args,count_type,te_tranlist):
	...
	for transcript in final_tranlist:
		for seq_type in transcript_dict[transcript]:
			...
			for inputfilename in transcript_dict[transcript][seq_type]:
				...
				input_list = [inputfilename, gene,transcript,region, count]
				if seq_type != "riboseq" and seq_type != "rnaseq":
	...
	header_str = "<thead><tr><th>Filename</th> <th>Gene</th> <th>Transcript</th> <th>Region</th><th>Count</th><th>View Plot</th></tr></thead>"
	table_str = "TE?~"+str(total_rows)+"?~"+header_str+"?~"+table_str
	return table_str
