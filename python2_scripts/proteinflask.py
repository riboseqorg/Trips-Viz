...
def get_fs_signals(seq):
    plusone_signals = ["AAATAAA","CCCT","CCCTGA","CTTAGG","CTTAGGC","CTTTAAC","CTTTGAC","GCGA","TCCTGA","TTTTGA","CTTAGTT"]
    minusone_signals = ["AAAAAAC","AAAAAAG","AAAAAAT","AAAAG","AAATTTA","AAATTTG","AAATTTT","AAATTTC","CCCAAAA","CCCAAAC",
                        "CCCAAAG","CCCTTTA","CGAAAG","GGATTTA","GGATTTT","GGGAAAA","GGGAAAC","GGGAAAG","GGGAAAT","GGGCCCC",
                        "GGGCCCT","GGGGAAC","GGGTTTA","GGGTTTT","GGTTTTC","GTTAAAC","TTTAAAA","TTTAAAC","TTTAAAT","TTTTTTA",
                        "TTTTTTC","TTTTTTG"]
    signal_dict = {"plus": {1:{0:"A"},2:{0:"A"},3:{0:"A"}},
                   "minus":{1:{0:"A"},2:{0:"A"},3:{0:"A"}}}
...
def plot_profile(tran, min_read,max_read,user_files_dict,lite,organism,nucseq,minscore): 
    user_proteo_files = []
    ...
    transhelve = shelve.open("/home/DATA/GWIPS_viz/Transcriptome_annotations/{0}/{0}.shelf".format(organism))
    ...
    start_stops = {1:{"starts":[0], "stops":[0]},
                   2:{"starts":[0], "stops":[0]},
                   3:{"starts":[0], "stops":[0]}}
    ...
    all_subcodon_reads = get_reads(min_read, max_read, tran, user_proteo_files, organism,tranlen,minscore)  
    try:
    ...
    font_axis = {'family': 'sans-serif', 'color': 'black', 'weight': 'bold', 'size': 20}
    ...
    allexons = ax2.plot((-1,-1), (0, 1), alpha=0.01,color='black',linestyle = '-.', linewidth=2)
    for exon in exon_junctions:
        allexons += ax2.plot((exon,exon), (0, y_max), alpha=0.01,color='black',linestyle = '-.', linewidth=3)
    ...
    if lite == "n":
        frame0subpro = ax2.fill_between(frame_counts[0].keys(), frame_counts[0].values(), alpha=0.75, label = labels, zorder=2, color= "#FF4A45", linewidth=0)
        frame1subpro = ax2.fill_between(frame_counts[1].keys(), frame_counts[1].values(), alpha=0.75, label = labels, zorder=2, color= "#64FC44", linewidth=0)
        frame2subpro = ax2.fill_between(frame_counts[2].keys(), frame_counts[2].values(), alpha=0.75, label = labels, zorder=2, color= "#5687F9", linewidth=0)
    else:
        frame0subpro = ax2.plot(frame_counts[0].keys(), frame_counts[0].values(), alpha=0.75, label = labels, zorder=2, color= "#FF4A45", linewidth=2)
        frame1subpro = ax2.plot(frame_counts[1].keys(), frame_counts[1].values(), alpha=0.75, label = labels, zorder=2, color= "#64FC44", linewidth=2)
        frame2subpro = ax2.plot(frame_counts[2].keys(), frame_counts[2].values(), alpha=0.75, label = labels, zorder=2, color= "#5687F9", linewidth=2)
    ...
    # draw cds start
    plt.plot((cds_start-2,cds_start-2), (0, y_max), 'gray',linestyle = 'solid',linewidth=2)
    
    # draw cds end
    plt.plot((cds_stop+3, cds_stop+3), (0, y_max), 'gray',linestyle = 'solid',linewidth=2)

    gs3 = gridspec.GridSpecFromSubplotSpec(5, 1, subplot_spec=gs[1], hspace=0.1)
    ...
    ax7.set_xlabel('Transcript: {} Length: {} nt'.format(tran, tranlen), fontdict=font_axis, labelpad=10)    
    ax7.set_ylabel('',fontdict={'family': 'sans-serif', 'color': 'black', 'weight': 'normal', 'size': '11'},
    ...
    for axisname in (ax4, ax5, ax6):
        axisname.tick_params(top=False, left=False, right=False, labeltop=False,
    ...
    for axis, frame in ((ax4, 1), (ax5, 2), (ax6, 3)):
        ...
        axis.broken_barh(starts, (0.5, 1), facecolors=start_colors,edgecolors=start_colors, label='start', zorder=5)
        stop_colors = [colors['stop'] for item in stops]
        axis.broken_barh(stops, (0, 1), facecolors=stop_colors,
                         edgecolors=stop_colors, label='stop', zorder=5)
        
        axis.set_ylabel('{}'.format(frame),fontdict={'family': 'sans-serif', 'color': colors['color'], 'weight': 'normal', 'size': '8'},
                        rotation='horizontal', labelpad=10, verticalalignment='center')
        ...
        ax5.set_ylim(0, 1)
        ax6.set_ylim(0, 1)
        axis.set_ylabel('{}'.format(frame),fontdict={'family': 'sans-serif', 'color': colors['color'], 'weight': 'normal', 'size': '6'},
                        rotation='horizontal', labelpad=10, verticalalignment='center')
    ...
    ilp = plugins.InteractiveLegendPlugin(line_collections, labels, alpha_unsel=0,alpha_sel=0.85)
