...
def nuc_comp(
    ...
    if nuc_comp_type == "nuc_comp_per":
        ...
        maxheight = max(max(master_dict["A"].values()),
    ...
    a_line = ax.plot(master_dict["A"].keys(),
                     ...
                     color=a_col,
                     linewidth=6)
    t_line = ax.plot(master_dict["T"].keys(),
                     master_dict["T"].values(),
                     label=labels,
                     color=t_col,
                     linewidth=6)
    g_line = ax.plot(master_dict["G"].keys(),
                     master_dict["G"].values(),
                     label=labels,
                     color=g_col,
                     linewidth=6)
    c_line = ax.plot(master_dict["C"].keys(),
                     master_dict["C"].values(),
                     label=labels,
                     color=c_col,
                     linewidth=6)
    ax.set_facecolor(background_col)
...
def mrna_dist_readlen(
    ...
    ax.set_title(title_str, y=1.05, fontsize=title_size)
    five_leader_line = ax.plot(mrna_dist_dict["5_leader"].keys(),
                               mrna_dist_dict["5_leader"].values(),
                               label=labels,
                               color='#ff6d6d',
                               linewidth=4)
    start_codon_line = ax.plot(mrna_dist_dict["start_codon"].keys(),
                               mrna_dist_dict["start_codon"].values(),
                               label=labels,
                               color='#a38b22',
                               linewidth=4)
    cds_line = ax.plot(mrna_dist_dict["cds"].keys(),
                       mrna_dist_dict["cds"].values(),
                       label=labels,
                       color='#00d8cd',
                       linewidth=4)
    stop_codon_line = ax.plot(mrna_dist_dict["stop_codon"].keys(),
                              mrna_dist_dict["stop_codon"].values(),
                              label=labels,
                              color='#05bc27',
                              linewidth=4)
    three_trailer_line = ax.plot(mrna_dist_dict["3_trailer"].keys(),
                                 mrna_dist_dict["3_trailer"].values(),
                                 label=labels,
                                 color='#ff77d4',
                                 linewidth=4)
...
def metagene_plot(
    ...
    if metagene_aggregate:
        ...
        if metagene_end == "metagene_five":
            ...
            rects1 = ax.bar(ind,
                            fiveprime_list,
                            width,
                            color=metagene_fiveprime_col,
                            linewidth=2,
                            edgecolor=metagene_fiveprime_col)
            start_pos = -300
        ...
        if metagene_end == "metagene_three":
            ...
            rects2 = ax.bar(ind + width,
                            threeprime_list,
                            width,
                            color=metagene_threeprime_col,
                            linewidth=2,
                            edgecolor=metagene_threeprime_col)
            start_pos = -300
        ...
        for file_id in count_dict:
            ...
            for val in count_dict[file_id]:
                ...
                i += 1
            plotcounts = ax.plot(pos_list,
                                 value_list,
                                 alpha=0.75,
                                 label=file_id,
                                 zorder=2,
                                 color=file_colors[file_ind],
                                 linewidth=2)
            file_ind += 1
    ...
    if metagene_type == "metagene_start":
        ax.set_xlabel("Position relative to cds start (nucleotides)",
                      labelpad=-10,
                      fontsize=axis_label_size)
    elif metagene_type == "metagene_stop":
        ax.set_xlabel("Position relative to cds stop (nucleotides)",
                      labelpad=-10,
                      fontsize=axis_label_size)
    title_str = "{} ({})".format(title, short_code)
...
def calc_trip_factor(read_dict):
    ...
    for i in range(0, len(read_dict["frame1"])):
        count_list = [
            read_dict["frame1"][i], read_dict["frame2"][i],
            read_dict["frame3"][i]
        ]
...
def trip_periodicity_plot(read_dict, title, short_code, background_col,
    ...
    for i in range(0, len(read_dict["frame1"])):
        ...
        returnstr += "{},{},{},{}\n".format(read_dict["readlengths"][i],
                                            read_dict["frame1"][i],
                                            read_dict["frame2"][i],
                                            read_dict["frame3"][i])
    ...
    plt.bar(pos,
            ...
            color=redhex,
            linewidth=0,
            label="frame 0")  #df['readlengths'][0])
    ...
    plt.ylim([
        0,
        max(max(df['frame1']), max(df['frame2']), max(df['frame3'])) * 1.1
    ])
...
def mapped_reads_plot(
    ...
    if not breakdown_per:
        ...
        p3 = plt.bar(
            ...
            bar_width,
            color='#e5584b',
            bottom=[i + j for i, j in zip(cutadapt_removed, rrna_removed)],
            linewidth=0)
        p4 = plt.bar(
            ind,
            ambiguous,
            ...
            bottom=[
                i + j + z
                for i, j, z in zip(cutadapt_removed, rrna_removed, unmapped)
            ],
            linewidth=0)
        p5 = plt.bar(ind,
                     mapped_noncoding,
                     ...
                     bottom=[
                         i + j + z + q
                         for i, j, z, q in zip(cutadapt_removed, rrna_removed,
                                               unmapped, ambiguous)
                     ],
                     linewidth=0)
        p6 = plt.bar(
            ...
            bottom=[
                i + j + z + q + b
                for i, j, z, q, b in zip(cutadapt_removed, rrna_removed,
                                         unmapped, ambiguous, mapped_noncoding)
            ],
            linewidth=0)
        p7 = plt.bar(ind,
                     pcr_duplicates,
                     ...
                     color='#01541e',
                     bottom=[
                         i + j + z + q + b + y for i, j, z, q, b, y in zip(
                             cutadapt_removed, rrna_removed, unmapped,
                             ambiguous, mapped_noncoding, mapped_coding)
                     ],
                     linewidth=0)
        p8 = plt.bar(ind,
                     totals,
        ...
        p3 = plt.bar(
            ...
            color='#e5584b',
            bottom=[
                i + j for i, j in zip(cutadapt_removed_per, rrna_removed_per)
            ],
            linewidth=0)
        p4 = plt.bar(ind,
                     ambiguous_per,
                     ...
                     bottom=[
                         i + j + z
                         for i, j, z in zip(cutadapt_removed_per,
                                            rrna_removed_per, unmapped_per)
                     ],
                     linewidth=0)
        p5 = plt.bar(
            ind,
            ...
            bottom=[
                i + j + z + q
                for i, j, z, q in zip(cutadapt_removed_per, rrna_removed_per,
                                      unmapped_per, ambiguous_per)
            ],
            linewidth=0)
        p6 = plt.bar(ind,
                     mapped_coding_per,
                     ...
                     color='#42951F',
                     bottom=[
                         i + j + z + q + b for i, j, z, q, b in zip(
                             cutadapt_removed_per, rrna_removed_per,
                             unmapped_per, ambiguous_per, mapped_noncoding_per)
                     ],
                     linewidth=0)
        p7 = plt.bar(
            ind,
            ...
            bottom=[
                i + j + z + q + b + y for i, j, z, q, b, y in zip(
                    cutadapt_removed_per, rrna_removed_per, unmapped_per,
                    ambiguous_per, mapped_noncoding_per, mapped_coding_per)
            ],
            linewidth=0)
        p8 = plt.bar(ind,
                     totals_per,
...
def explore_offsets(
    ...
    for i in range(0, len(f0_counts)):
        if i % 3 == 0:
            ...
            width = -0.4
        p1 = plt.bar(ind[i] + width,
                     f0_counts[i],
                     bar_width,
                     color=greenhex,
                     linewidth=0)
        p2 = plt.bar([pos[i] + width],
                     f1_counts[i],
                     bar_width,
                     color=redhex,
                     bottom=f0_counts[i],
    ...
    for filetype in file_paths_dict:
        for file_id in file_paths_dict[filetype]:
            ...
            if os.path.isfile(filepath):
                ...
                return_str = "File not found: {}, please report this to tripsvizsite@gmail.com or via the contact page.".format(
                    filepath.split("/")[-1])
                return return_str
            if "frequent_unmapped_reads" not in sqlite_db:
                return_str = "No unmapped reads data for {}, please report this to tripsvizsite@gmail.com or via the contact page.".format(
                    filepath.split("/")[-1])
                return return_str
...
def heatplot(min_readlen, max_readlen, min_pos, max_pos, positions,
    ...
    for i in range(min_pos, max_pos):
        xlabs.append(i)
    df = pd.DataFrame(np.column_stack([positions, readlengths, count_list]),
                      columns=['positions', "readlengths", "counts"])
    ...
    if color_palette == "gist_rainbow":
        ...
        color_pallete_list = all_palettes[color_palette][max(
            all_palettes[color_palette].keys())]
    ...
    p = figure(plot_width=1300,
               plot_height=750,
               x_range=(min_pos, max_pos),
               y_range=(min_readlen, max_readlen),
               title="Heatmap ({})".format(short_code),
               y_axis_label="Readlengths",
               toolbar_location="below",
...
def rust_dwell(codon_count_dict, short_code, background_col, title_size,
    ...
    p = figure(
        plot_width=1300,
        plot_height=750,
        y_axis_label='RUST ratio (relative to min)',
    ...
    for tup in sorted_list:
        ...
        for codon in aa_dict[aa]:
            codons.append(codon)
            x_vals.append(x_val)
            rust_ratio = (float(codon_count_dict[codon] + 0.000001) /
                          min_count)
            y_vals.append(rust_ratio)
            mytext = Label(x=x_val - 0.2,
                           y=-10,
                           text=aa,
                           background_fill_color=aa_color_dict[aa],
                           text_font_size="18pt")
            p.add_layout(mytext)
        x_val += 1
...
def calc_mrnadist_factor(mrna_dist_dict):
    ...
    for key in mrna_dist_dict:
        for region in mrna_dist_dict[key]:
            mrna_dist_dict[key][region] = float(
                mrna_dist_dict[key][region]) / factor
...
def mrna_dist(
    ...
    for key in mrna_dist_dict:
        ...
        total = float(mrna_dist_dict[key]["5_leader"] +
                      ...
                      mrna_dist_dict[key]["3_trailer"])
        labels.append(key)
        five_leaders.append(mrna_dist_dict[key]["5_leader"])
        five_leaders_per.append(
            (mrna_dist_dict[key]["5_leader"] / total) * 100)
        ...
        if md_start and md_stop:
            ...
            cds.append(mrna_dist_dict[key]["cds"] +
                       mrna_dist_dict[key]["stop_codon"])
            cds_per.append(((mrna_dist_dict[key]["cds"] +
                             mrna_dist_dict[key]["stop_codon"]) / total) * 100)
            ...
            start_codons_per.append(0)
        returnstr += "{},{},{},{},{},{}\n".format(
            key, mrna_dist_dict[key]["5_leader"],
            mrna_dist_dict[key]["start_codon"], mrna_dist_dict[key]["cds"],
            mrna_dist_dict[key]["stop_codon"],
            mrna_dist_dict[key]["3_trailer"])
    labels.append("")
    ...
    if not mrna_dist_per:
        ...
        p3 = plt.bar(
            ...
            bar_width,
            color='#00d8cd',
            bottom=[i + j for i, j in zip(five_leaders, start_codons)],
            linewidth=0)
        p4 = plt.bar(ind,
                     stop_codons,
                     bar_width,
                     ...
                     bottom=[
                         i + j + z
                         for i, j, z in zip(five_leaders, start_codons, cds)
                     ],
                     linewidth=0)
        p5 = plt.bar(ind,
                     three_trailers,
                     ...
                     color='#ff77d4',
                     bottom=[
                         i + j + z + q for i, j, z, q in zip(
                             five_leaders, start_codons, cds, stop_codons)
                     ],
                     linewidth=0)
    else:
        p1 = plt.bar(ind,
        ...
        p3 = plt.bar(
            ...
            bar_width,
            color='#00d8cd',
            bottom=[i + j for i, j in zip(five_leaders_per, start_codons_per)],
            linewidth=0)
        p4 = plt.bar(
            ind,
            stop_codons_per,
            ...
            bottom=[
                i + j + z
                for i, j, z in zip(five_leaders_per, start_codons_per, cds_per)
            ],
            linewidth=0)
        p5 = plt.bar(
            ind,
            ...
            bottom=[
                i + j + z + q
                for i, j, z, q in zip(five_leaders_per, start_codons_per,
